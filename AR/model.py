...
class CosRec(nn.Module):
    ...
    Args:
        num_users: number of users.     (in all db, not just a minibatach)
        num_items: number of items.     (in all db)
    ...
    '''
    def __init__(self, num_users, num_items, embed_dim, block_num, block_dim, fc_dim, ac_fc, drop_prob):
        ...
        self.item_embeddings.weight.data[0]=0
    ...
    def forward(self, seq_var, user_var, item_var, for_pred=False):
        ...
        mb = seq_var.shape[0]
        seq_len=seq_var.shape[1]
        item_embs = self.item_embeddings(seq_var) # (b, L, embed)               (b, 40, 50)
        user_emb = self.user_embeddings(user_var) # (b, 1, embed)               

        # add user embedding everywhere
        usr = user_emb.repeat(1, seq_len, 1) # (b, 5, embed)
        ...
        all_embed=item_embs.reshape(mb,-1,4,2*self.embed_dim)                 #torch.Size([512, 5, 4, 100])
        out = all_embed.permute(0, 3, 1, 2)                                   #torch.Size([512, 100, 5, 4])

        # 2D CNN
        for i in range(self.block_num):
            out = self.cnnBlock[i](out)
        out = self.maxpool(out).reshape(mb, self.cnnout_dim)
...
class cnnBlock(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, stride=1, padding=0):
        ...
        self.conv2 = nn.Conv2d(output_dim, output_dim, kernel_size=2, stride=1)
        self.relu = nn.ReLU()
