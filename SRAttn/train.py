    ...
    def _initialize(self, interactions):
        ...
        if self.model_type == 'mlp':
            ...
            self._net = CosRec(self._num_users, self._num_items, self.args.L, 
                ...
                drop_prob=self.args.drop, num_heads=self.args.num_heads).to(self._device)
    ...
    def fit(self, train, test, verbose=False):
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            for (minibatch_num,
                ...
                #compute warp loss
                # loss = l.warp_loss(torch.tanh(targets_prediction), torch.tanh(negatives_prediction), num_labels=self._num_items, device=torch.device('cuda'))
                # epoch_loss += loss.item()
            ...
            if verbose and (epoch_num + 1) % 2 == 0:
                ...
                if mean_aps > best_map:
                    best_map = mean_aps
                    checkpoint_name = "attention_ml1m_test.pth.tar"                            #CHECK
                    save_checkpoint({
                    'epoch': epoch_num+1,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_cuda', type=str2bool, default=True)                               #CHECK
    # save path
