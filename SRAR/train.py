...
class Recommender(object):
    ...
    def _initialize(self, interactions):
        ...
        self.test_sequence = interactions.test_sequences                              # using last subsequence of each user
    ...
    def fit(self, train, test, verbose=False):
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            for (minibatch_num,
                ...
                items_to_predict = torch.cat((batch_targets, batch_negatives), 1)       #torch.Size([509, 6])  
                                                                                        #print(items_to_predict[0]) tensor([  51,   14,   54,   77, 3169,  689])
                items_prediction = self._net(batch_sequences,
                                             batch_users,
                                             items_to_predict)                           #torch.Size([509, 6]) 
                                                                                        # print(items_prediction[0]): output likelihoods of items in items_to_predict
                                                                                        #tensor([ 4.1990,  5.6241,  4.2237,  0.0341, -2.6239, -1.0213]
            ...
            if verbose and (epoch_num + 1) % 2 == 0:
                ...
                if mean_aps > best_map:
                    best_map = mean_aps
                    checkpoint_name = "test_ars_ml1m.pth.tar"                              #ALWAYS CHECK IF ITS NEW
                    save_checkpoint({
                    'epoch': epoch_num+1,
    ...
    def _generate_negative_samples(self, users, interactions, n):
        ...
        if not self._candidate:
            ...
            for user, row in enumerate(train):
                self._candidate[user] = list(set(all_items) - set(row.indices))   #get the list of items that a user never interacted with
    ...
    def predict(self, user_id, item_ids=None):
        ...
        with torch.no_grad():
            ...
            out = self._net(sequences,              #using test sequence (last subsequence for each user)
                            user,
                            items,
                            for_pred=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-3)  #1e-3     1e-4
    parser.add_argument('--l2', type=float, default=5e-6)       #5e-6
    ...
    parser.add_argument('--d', type=int, default=50)    #50
    ...
    parser.add_argument('--drop', type=float, default=0.5, help='drop out ratio.')  #0.5    0.4-0.6
    parser.add_argument('--fc_dim', type=int, default=150)    #150
    parser.add_argument('--ac_fc', type=str, default='tanh',                  #'tanh'
                                choices=['relu', 'tanh', 'sigm'])
    parser.add_argument('--min_support', type=float, default=0.000013)    
    ...
    set_seed(args.seed,
             cuda=args.use_cuda)
    # load dataset
    train = Interactions(args.data_root+args.dataset+args.train_dir, args.min_support, args.min_threshold)
    # transform triplets to sequence representation
    train.to_sequence(args.L, args.T)                   
    

    test = Interactions(args.data_root+args.dataset+args.test_dir,  args.min_support, args.min_threshold,     
